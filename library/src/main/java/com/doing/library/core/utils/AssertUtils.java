...
/**
 * Assert类，Null检查，对象检查，数组检查等
 *
 * 检查数据合法性，抛出异常
 *
 * RuntimeException  ：可以不用catch ; Checked Exception （编译时异常）：必须catch
 * 本类中的IllegalArgumentException 为 RuntimeException，可以不用catch
 */
...
public abstract class AssertUtils {
    ...
    public static void isTrue(boolean expression, String message) {
        if (!expression) {
            throw new IllegalArgumentException(message);  //RuntimeException
        }
    }
